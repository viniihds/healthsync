...
const getAdmin = async user => {
    ...
    try {
        const sql = `SELECT U.CDUSER, NMUSER, PASSWORD
                     FROM USERS U
                     INNER JOIN USERPERMISSION UP ON UP.CDUSER = U.CDUSER
                     INNER JOIN PERMISSION P ON P.CDPERMISSION = UP.CDPERMISSION
                     WHERE EMAIL = ? AND PASSWORD = ? AND IDPERMISSION = ${permissionEnum.ADMIN};`;
